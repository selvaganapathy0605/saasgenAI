    ...
    <div onClick={() => setExpanded(!expanded)} className='p-4 max-w-5xl text-sm bg-white border border-gray-200 rounded-lg cursor-pointer'>
      <div className="flex justify-between items-center gap-4">
        <div className="">
          ...
          <p children="text-gray-500">{item.type} - {new Date(item.created_at).toLocaleDateString()}</p>
        </div>
        <button className='bg-[#EFF6FF] border border-[#BFDBFE] text-[#1EA0Af] px-4 py-1 rounded-full'>{item.type}</button>
      </div>
      {
        expanded && (
          <div className="">
