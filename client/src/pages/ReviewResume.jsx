  ...
  return (
    <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
      <form onSubmit={handleSubmit} className="w-full max-w-lg bg-white p-4 rounded-lg border border-gray-200">
        ...
        <input onChange={(e) => setResume(e.target.files[0])} accept='application/pdf' type="file" className='text-gary-600 w-full p-2 px-3 mt-2 outline-none text-sm rounded-md border border-gray-300 hover:border-blue-300' required />
      ...
      <div className='w-full max-w-lg p-4 bg-white rounded-lg flex flex-col border border-gray-200 min-h-96 max-h-[600px]'>
        ...
        <div className="flex items-center justify-between">
          ...
          {content && (
            <button
              onClick={handleCopy}
              className="text-xs px-3 py-1 border rounded-md hover:bg-green-100 hover:border-green-500"
            >
              Copy
            </button>
          )}
